...
if import_data:
    ...
    input_layer_size = FRAME_SIZE  # еще добавляются амплитуды
    output_layer_size = MOTION_COUNT

    nn.add_input_layer(input_layer_size)  # добавляем входной слой
    nn.add_hidden_layer(24)  # добавляем скрытый слой
    nn.add_hidden_layer(12)  # добавляем скрытый слой
    nn.add_hidden_layer(6)  # добавляем скрытый слой
    ...
    for i in range(0, TRAIN_EPOCH_COUNT):
        ...
        loss_total = nn.train_accumulate(train_data, 0.1, False)
